...
| T-A24 | **SUSPECT band** (v1.71, §5.4.2) | With the base in IDLE, connect a ~750 Ω resistor (or a deliberately corroded/high-resistance joint) to a channel: the remote grid shows **◆ SUSPECT in orange**, the strip pixel is steady orange, and the base log reports the band as SUSPECT — **no blip sounds** (silence is the specified treatment, §12.2). Long-press ARM on that channel: NACK `0x10`, remote displays **"HIGH RESISTANCE ON CH N"** — not "NO CONTINUITY". Then the disarm half: arm a *good* channel, and swap its load for the ~750 Ω one — the base disarms within ~1 s (§7.2.7), siren `SIREN_CONTINUITY_LOST`, remote shows the channel ◆ SUSPECT. Boundary sanity: a ~470 Ω load reads MARGINAL (below the 500 Ω boundary) and a ~1.5 kΩ load reads OPEN (above saturation) — confirming the SUSPECT window is where specified and neither wider nor narrower. **PARTIAL PASS 2026-09-15 (fw 1.2.20, both units):** the boundary half is confirmed — 1 kΩ → ◆ SUSPECT and 1.2 kΩ → ○ OPEN on adjacent channels (the split sits exactly between them, matching the ~1.09 kΩ derivation), the all-channels-idle resting display still reads ○ OPEN on both units, and the OPEN-side ARM refusal NACKs 0x04 (T-A04, re-verified). **Still outstanding:** the 750 Ω no-blip + NACK `0x10` refusal on a SUSPECT channel, the disarm-on-degradation swap while ARMED, and the 470 Ω MARGINAL sanity value. |
