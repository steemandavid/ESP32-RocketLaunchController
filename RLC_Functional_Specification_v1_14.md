...
| 1.48 | 2026-08-28 | **Documentation sweep: LINK_REQUEST-while-busy wording.** Three places (§6.4.1, §7.2.6 POST_FIRE exceptions, App D exception table) still said the base "silently ignores" a LINK_REQUEST received while ARMED/PRE_FIRE/FIRING/POST_FIRE, describing the pre-1.1.17 behaviour — the §15.4 T-S09 row had already been amended but these had not. All three now state the actual behaviour: the base rejects with `LINK_REJECT_BUSY` (0x02), no session consequences, remote retries non-terminally. Code unchanged. Also fixed in the same sweep: the first-shot-of-a-power-cycle `gptimer_stop` ERROR log is silenced in firmware (a "timer ever started" flag), and the Development_Progress Phase Overview table is brought up to date. |
...
| 1.71 | 2026-09-14 | **Fourth continuity band `CONT_SUSPECT` (operator request); firmware 1.2.19 → 1.2.20.** Splits the old OPEN territory at new `CONT_SUSPECT_UV` (928000 µV, ~1.09 kΩ): 500 Ω–1.09 kΩ now reports **SUSPECT** — a finite, measurable resistance where OPEN used to swallow it — and OPEN is reserved for readings at/near ADC saturation (~1.14 kΩ and up, resting at the R_pull rail). Purpose: an igniter connected through badly corroded clips or a damaged lead previously read identically to *no igniter at all*; the operator was told "NO CONTINUITY ON CH N" while staring at a physically connected igniter. SUSPECT **blocks arming** exactly as OPEN does (anything above the 500 Ω MARGINAL ceiling cannot fire) but carries its own NACK `0x10` ("HIGH RESISTANCE ON CH N") and disarms per §7.2.7 when it appears on the armed channel — the rule is now "any band that blocks arming disarms", still met by OPEN and SUSPECT alike. Presentation: steady **orange** (255,140,0 — the retired SHORT colour, §10.2.0) and the retired diamond glyph ◆ (§10.2.2 grid, §10.2.4a fire-complete line, RGB strip pixel). Siren: **silent**, like OPEN. Wire encoding: **enum value 3, the retired SHORT slot, is repurposed** — safe because strict firmware-version matching (§6.2, T-C07) means no pre-1.2.20 peer can ever be linked; no struct change, no `protocol_version` bump. Honest hardware limit recorded in §5.4.2: above ~1.14 kΩ the 0 dB ADC range saturates and SUSPECT is indistinguishable from absent — the band covers 500 Ω–1.09 kΩ only. §5.4.2, §5.4.6, §6.3 (STATUS_UPDATE field), NACK table, §7.2.2, §7.2.7, §7.3.1, §9.2, §10.2.0, §10.2.2, §10.2.4a, §12.2, §14.5, §15.2 (T-A09, new T-A24) and §15.5 (T-L02) updated. Both units must be flashed together. |
| 1.72 | 2026-09-15 | **v1.71 validated on target; OPEN-side results recorded.** Both units on fw 1.2.20 (12/12 boot self-test suites each, link rssi −25, 0 missed pings). The SUSPECT/OPEN boundary is confirmed exactly where derived: **1 kΩ → ◆ SUSPECT and 1.2 kΩ → ○ OPEN** on adjacent channels — the split sits between them, at the ~1.09 kΩ boundary, and saturated-but-finite resistance correctly reports OPEN rather than claiming a measurement it cannot make. The all-idle resting display still reads ○ OPEN on all eight channels. **T-A04 re-verified:** OPEN still NACKs 0x04, distinct from 0x10. **T-A16 re-verified** after the v1.71 edit to `armed_channel_went_open()` (now also accepts SUSPECT): pulling the igniter while ARMED disarms within ~1 s with the 3 × 200 ms blasts, and the key-off disarm remains acoustically distinguishable. **T-S19 screen wording re-verified:** `OPEN - LIKELY FIRED` in green, SUSPECT row not disturbing it. **T-A24 PARTIAL PASS** — boundary half done; outstanding: the 750 Ω no-blip + 0x10 refusal, the disarm-on-degradation swap while ARMED, and the 470 Ω MARGINAL sanity value. Test-first session note: the boot self-test halted the first 1.2.20 flash fail-safe on a bad hysteresis vector (a check starting from the wrong prior band) — the Phase-2 M2 design catching exactly the divergence it exists for. No firmware change this revision; documentation only. |
| 1.73 | 2026-09-15 | **T-A24 fully PASS; v1.71 SUSPECT band closed out on target.** As-run values with the bench parts available (recorded in the T-A24 row): **820 Ω** (~783 mV, mid-window) for the SUSPECT cases — ◆ orange and silent on connect, NACK `0x10` "HIGH RESISTANCE" on ARM, and the **CONNECTED→SUSPECT disarm live on target** (armed on a 2 Ω load, 820 Ω bridged across, 2 Ω removed → disarm within ~1 s, 3 × 200 ms blasts, "CONTINUITY LOST - DISARMED", channel settles ◆ SUSPECT). **390 Ω** (~510 mV) → ▲ MARGINAL with the two-blip signal for the lower-boundary sanity check. Two procedural notes now in the test row: the disarm-half swap must have **no open gap** (bridging first, then removing — simply unplugging exercises the T-A16 OPEN path and proves nothing about SUSPECT, which the first run of this test demonstrated), and **560 Ω must not be substituted for the MARGINAL check** — it reads ~625 mV and lands ◆ SUSPECT, which would look like a boundary shift that is not there. With this, every v1.71 behaviour — classification, refusal reason, disarm, presentation, and both boundaries — has been exercised on the hardware. No firmware change; documentation only. |
...
| **Continuity** | Analogue measurement of the igniter circuit resistance via ADC. The continuity sensing circuit uses the SPDT relay's NC contact to route the igniter to the sense circuit when the relay is de-energised. Uses ≤ 1 mA test current. Results are classified into four bands: CONNECTED, MARGINAL, SUSPECT, OPEN (the former SHORT band was folded into CONNECTED, v1.29; SUSPECT split out of OPEN, v1.71). |
| **Continuity Band** | One of four classifications derived from the ADC continuity measurement: CONNECTED (< ~67 Ω, low-resistance path present — includes a dead short, which the measurement cannot distinguish), MARGINAL (~67–500 Ω, high resistance connection), SUSPECT (~500 Ω–1.09 kΩ, finite but unreasonably high resistance — corroded contacts, damaged lead; blocks arming), OPEN (> ~1.09 kΩ or no igniter — at/near ADC saturation). Boundaries are the `CONT_MARGINAL_UV` / `CONT_OPEN_UV` / `CONT_SUSPECT_UV` thresholds of §14.5. |
...
**FOUR bands (revised 2026-09-14, v1.71).** `CONT_SHORT` was merged into `CONT_CONNECTED` (2026-08-21); `CONT_SUSPECT` was split out of OPEN (v1.71) — see the notes below.

| Band | Enum | R_ign range | V_adc range | Meaning | Display | Arming |
|---|---|---|---|---|---|---|
| `CONT_OPEN` | 0 | > ~1.09 kΩ / ∞ | > `CONT_SUSPECT_UV` (928000 µV) | No measurable path. Pulled to ~3.19 V by R_pull, saturating the 0 dB ADC range — saturated means OPEN. | Yellow ○ | **Blocks arming** (NACK 0x04) |
...
| `CONT_SUSPECT` | 3 | ~500 Ω–1.09 kΩ | `CONT_OPEN_UV` to `CONT_SUSPECT_UV` | A finite, measurable resistance too high to fire — badly corroded contacts, a partly severed lead. **Something is connected** where OPEN used to report "no igniter". | Orange ◆ + "SUSPECT" | **Blocks arming** (NACK 0x10) |
| ~~`CONT_SHORT`~~ | ~~3~~ | — | — | **RETIRED 2026-08-21, slot reused for `CONT_SUSPECT` in v1.71** (see the wire-encoding note below). | — | — |
...
> **Why SUSPECT was added (2026-09-14, v1.71).** Before the split, everything above ~500 Ω reported OPEN — so an igniter connected through badly corroded clips read identically to *no igniter at all*, and the operator was told "NO CONTINUITY ON CH N" while staring at a physically connected igniter. The 0 dB ADC range gives a measurable window up to ~1.14 kΩ (full scale, 950 mV): within 500 Ω–1.09 kΩ the resistance is finite and quantifiable, and that is precisely where a few hundred ohms of contact corrosion lands when added to an igniter's own 1–2 Ω. SUSPECT reports it as "connected, but unreasonably high — go look at the contacts" instead of "absent". **Honest hardware limit:** above ~1.14 kΩ the reading saturates at raw 4095 and SUSPECT is indistinguishable from absent. Extending the range would require more attenuation, which collapses the resolution where real igniters live (the 12 dB trial, §14.5 `CONT_ADC_ATTEN`) — the band therefore deliberately covers 500 Ω–1.09 kΩ only. A band that cannot be measured must not be reported; equally, a measurement that *can* be made must not be thrown away.

**Wire encoding alignment:** the enum values (CONT_OPEN=0, CONT_CONNECTED=1, CONT_MARGINAL=2, CONT_SUSPECT=3) are identical to the 2-bit wire encoding in the `continuity_bands` field of STATUS_UPDATE. No mapping function is required. **Value 3 is the retired SHORT slot, repurposed for SUSPECT in v1.71** — safe because strict firmware-version matching (§6.2, T-C07) refuses to link any peer that is not running the identical build, so no pre-v1.71 receiver can ever decode a value 3 from this firmware. The fold-a-pre-merge-peer's-value-3-into-CONNECTED logic is deleted with the slot's reuse. No struct change, no `protocol_version` bump.

**Hysteresis:** each threshold has a configurable hysteresis band (default: ±5000 µV for the MARGINAL boundary, ±50000 µV for the SUSPECT lower boundary, ±10000 µV for the SUSPECT/OPEN boundary). A band transition is only registered when the averaged voltage crosses the threshold + hysteresis in the transition direction. This prevents oscillation at band boundaries. **The SUSPECT/OPEN hysteresis is deliberately narrow** because its OPEN side is pinned: a saturated channel reads exactly full scale (950000 µV), so the threshold + hysteresis must stay below 950000 µV for a true open to ever cross it — 928000 + 10000 = 938000 µV leaves 12 mV of margin.

**SHORT band reliability (DEPRECATED with the SHORT band, v1.29):** the SHORT/GOOD boundary at ~500 µV is near the ESP32-S3 ADC noise floor, and this analysis predates the merge — the bands are now CONNECTED, MARGINAL, SUSPECT, OPEN. Retained as a record of why the distinction was abandoned (see the bench-evidence note above).
...
>
> The mapping has two independent confirmations. **The photograph**, taken with
> the base **idle and the key at ARM** (base log `arm=0 key=1`, state IDLE): the
> lower-right lamp is lit red, the lower-centre lamp is dark, and HOT is dark —
> the key alone does not make the pad live, which is the two-break design working
> as specified. **The Visio drawing**, whose own annotations name the lamps
> `SAFE LED`, `ARM LED` / `ARMED LED` and `HOT`, and place SAFE and ARM at the
> same height with SAFE to the left of ARM, and HOT higher and further left —
> the same geometry the photograph shows.
>
...
> the antenna at the left to the on/off plate at the right.
>
> The whole-panel Visio drawing that v1.56 cited (`Base front panel layout.svg`,
> 10.4 MB, on `/mnt/netwerkdrive`) does carry annotations reading **"Usb com
> connector"** and **"Usb jtag connector"** — so the claim had a source. But
> those two annotations sit in the drawing's **interior** group, alongside
> `battery`, `siren` and `XT60 battery connector`, not with the panel-mounted
> items (`On/off switch`, the key-plate LEDs, `antenna`). The drawing is
> annotating the **ESP32-S3 board's own two ports inside the case** — the UART
> bridge used as COM, and native USB — which is exactly what the photograph
> shows. v1.56 read an interior annotation as a panel feature.
>
> A service cable is routed in over the top edge of the panel. v1.54's original
> position — service access by opening the case, charger not brought out — is
> reinstated and extended to both ports.
>
...
After a fire pulse completes and the channel relay returns to its NC position, the continuity sensing circuit automatically reconnects to the igniter. The arm relay is also de-energised on transition to POST_FIRE, breaking the fire path for all channels. The continuity task resumes meaningful ADC sampling for that channel (continuity sense uses its own 3.3V supply through R_ref, independent of the arm relay state). An igniter that has fired will read as OPEN (high resistance / open circuit; occasionally SUSPECT if combustion residue leaves a finite resistance — both mean "no useful continuity"). An igniter that failed to fire will still read as CONNECTED or MARGINAL.
...
fault SHALL also be shown **for as long as it stands**, not only when it
appears, and the ARM and FIRE refusals SHALL name it rather than instructing the
operator to turn the key: see **§10.2.7a**, which exists because the first
implementation (v1.59) did neither. The
...
| Range | **Measured 2026-08-25: 200 m holding at −93 dBm. Measured 2026-09-10: 430 m holding at −91 dBm** — both with external antennas on both units, base on the ground. The 430 m result **exceeds the two-ray (d⁴) extrapolation of the 200 m point** (~250–260 m in that geometry, ~−106 dBm at 430 m) by ~15 dB, so the d⁴ model is now recorded as a **pessimistic bound, not a prediction**: it fitted the single 200 m measurement to 0.5 dB, but the real link's geometry (remote height, ground conditions) evidently departs from the ideal two-ray case. At 430 m the link still had **5–8 dB of margin** against the drop-out. Expected drop-out **−96 to −99 dBm** (1 Mbps DSSS sensitivity ≈ −98 dBm, extended 1–3 dB by the 3-consecutive-miss rule); `ERR_COMM_DEGRADED` refuses arming a couple of dB before that. Raising both ends to ~1.5 m remains worth ~30 dB and would push the link past 1 km. RSSI figures from different sessions are not strictly comparable — the reading is uncalibrated and compressed in the high −90s (v1.33). |
...
| Offset | Size | Field | Description |
|---|---|---|---|
| 0 | 2 | `continuity_bands` | 2 bits per channel (ch1 in bits 1:0, ch2 in bits 3:2, ... ch8 in bits 15:14). Values: 00 = OPEN, 01 = CONNECTED, 10 = MARGINAL, 11 = SUSPECT *(the retired SHORT slot, repurposed v1.71 — see §5.4.2; safe under strict version matching)*. Enum values match wire encoding directly (CONT_OPEN=0, CONT_CONNECTED=1, CONT_MARGINAL=2, CONT_SUSPECT=3). No struct change, so no protocol version bump was required. |
...
| `0x10` | Channel reads SUSPECT — finite but unreasonably high resistance (~500 Ω–1.09 kΩ; v1.71). The igniter appears connected; check clips, contacts and leads before retrying | "HIGH RESISTANCE ON CH N" |
...
If the base receives a `LINK_REQUEST` while already linked to the same remote MAC (e.g., after a remote reboot), it shall **atomically invalidate the previous session token**, reset both per-peer sequence counters to 0, generate a new session token, and respond normally. **However, the base SHALL reject a LINK_REQUEST received while in ARMED, PRE_FIRE, FIRING, or POST_FIRE state with `LINK_REJECT_BUSY` (0x02)** rather than silently dropping it (amended fw 1.1.17; the original "silently ignores" wording predates that). A session reset during these states would invalidate the active session and disrupt a safety-critical operation. The rejection carries no session consequences — no `LINK_ACK`, no new token, no dropped arm — and the base remains in its current state and continues processing the existing session. The remote treats `LINK_REJECT_BUSY` as non-terminal and keeps retrying every 2000 ms, so a subsequent request is delivered after the base returns to IDLE (via disarm, fire completion, link-loss timeout, or POST_FIRE cooldown), where the handshake is accepted normally. Verified on target by T-S09.
...
- Guard conditions (ALL must be true):
  ...
  2. Channel N has continuity (band is CONNECTED or MARGINAL — OPEN **and SUSPECT** block arming; v1.71. SUSPECT NACKs `0x10` rather than `0x04` so the operator is told the igniter *reads connected but unreasonably high*, not that it is absent).
...
- Exceptions:
  ...
  - **LINK_REQUEST received during POST_FIRE:** the base SHALL reject it with `LINK_REJECT_BUSY` (0x02), as in ARMED/PRE_FIRE/FIRING. The remote's retry mechanism (every 2000 ms) will deliver a subsequent request after the base returns to IDLE. The cooldown is not interrupted — relays are already safe and the timer ensures a minimum delay before the next arming sequence.
...
- **Continuity on the armed channel transitions to OPEN or SUSPECT** while in ARMED or PRE_FIRE (OPEN added v1.35, 2026-08-26; SUSPECT added v1.71 — see below).
...
1. Only the bands that **block arming** SHALL trigger disarm — **OPEN and SUSPECT** (v1.71), matching the arming guard (§7.2.2 guard 2), which refuses on the same two. MARGINAL remains informational (§7.3.1 step 2). An armed channel that degrades to SUSPECT (~500 Ω–1.09 kΩ) will not fire and SHALL be treated exactly as an open one.
...
5. The remote learns of the disarm through the resulting `STATUS_UPDATE`, whose `continuity_bands` field shows the channel as OPEN (or SUSPECT, v1.71). No new NACK reason is defined for the disarm itself — there is no command to NACK, since the trigger is a spontaneous hardware event.
...
A dedicated FreeRTOS task (`continuity_task`) shall sample all 8 continuity ADC inputs in round-robin fashion. Each channel is sampled every 100 ms using the `adc_cali_raw_to_voltage()` calibration API, with 64-sample oversampling per reading for noise reduction. The calibrated millivolt value is classified into a continuity band (CONNECTED, MARGINAL, SUSPECT, OPEN — the former SHORT band was folded into CONNECTED in v1.29; SUSPECT split out of OPEN in v1.71) using the threshold constants from §14.5 with hysteresis.

When a band change is detected on any channel:
1. Update the internal `continuity_bands` field.
2. If a channel that is currently armed transitions to MARGINAL: log an advisory warning (informational only — does not trigger disarm). Only OPEN and SUSPECT disarm (v1.71), matching the arming guard. (This step named the SHORT band until v1.44; that band was folded into CONNECTED in v1.29 and is never produced.)
3. Trigger an event-driven `STATUS_UPDATE` to the remote.
4. Post the change — channel number, new band **and** an `initial` flag — to the base FSM event queue as `EVT_CONTINUITY_CHANGED`, so the state machine can apply §7.2.7's continuity-loss disarm. The band is carried in the event rather than re-read by the FSM, because the round-robin sampler may have moved on by the time the event is dequeued. The `initial` flag (v1.57) marks a channel's **first** classification since boot or since an ADC failure recovered — the sampler settling on reality rather than the world changing — because all bands start at OPEN, so an igniter already connected at power-on produces a change event that nobody caused. The post SHALL use a short blocking send rather than a zero-timeout one: dropping this event on a transient queue burst would silently leave the base armed on an open igniter.
5. **Igniter-connection blips (v1.57/v1.58, §12.2).** On a transition to **CONNECTED** the FSM SHALL sound `SIREN_IGNITER_CONNECTED` (one `SIREN_CONNECT_CHIRP_MS` blip); on a transition to **MARGINAL** it SHALL sound `SIREN_IGNITER_MARGINAL` (two such blips). One blip means the connection is good, two mean it is high-resistance and should be inspected — the operator working at the motor hears the difference without walking back to the LEDs, which is the case that costs a launch window. **OPEN and SUSPECT are silent** (SUSPECT added v1.71): disconnection is not news to the person doing it, and a SUSPECT reading is not a connection made — in both cases the operator's next move is the same (look at the display, fix or re-make the joint), so no new acoustic signal is defined. The armed-channel case has its own, louder treatment in §7.2.7. This is audible-only: it changes no state and gates nothing.

   The blips SHALL sound **only in BOOT and IDLE**. BOOT is included deliberately — igniters are routinely connected before the LCO has powered the remote, and the base waits in BOOT until the link comes up. They SHALL NOT sound in ARMED, PRE_FIRE or FIRING, where the siren is the pad's continuous warning that the fire path is live and where the one-shot's drive-on/drive-off mechanism would silence it; nor in LINK_LOST or ERROR, where a patterned alert is running and carries meaning; nor in POST_FIRE, where the fired channel's re-read is the fire-result indication rather than an operator action. The siren driver SHALL additionally decline a blip whenever the siren is already sounding — the state gate and the driver guard are both required, so that the pad warning cannot be silenced by a property of one switch statement.

   `SIREN_IGNITER_MARGINAL` SHALL remain acoustically distinct from `SIREN_ERROR` and `SIREN_CONTINUITY_LOST`: two short blips against three long blasts. The alert patterns mean *stop*; these mean *look*. Any future change to either pair SHALL preserve that distinction.

   Three suppressions keep the blips meaning exactly "somebody just connected an igniter": an event carrying the `initial` flag never sounds (the sampler settling on a channel it had never read); no blip SHALL sound within `SIREN_CONNECT_CHIRP_INHIBIT_MS` of the POST_FIRE → IDLE transition, where an igniter that failed to fire reappears as CONNECTED or MARGINAL with nobody having touched it (see the post-fire igniter status paragraph below); and a channel SHALL NOT repeat **the same** signal within `SIREN_CONNECT_CHIRP_MIN_INTERVAL_MS`.

   That last rate limit is **per channel and per band** (v1.58). A repeat of the same signal inside the window is chatter — a half-seated connector being wiggled — and says nothing the operator has not just heard. A **change** of signal SHALL always sound, because "two blips → re-seat the crimp → one blip" is the loop these signals exist to close, and a band-blind window would swallow the confirming blip and leave the operator believing the crimp was still bad. The consequence is accepted deliberately: a connection oscillating across the CONNECTED/MARGINAL boundary can blip on each change. The round-robin sampler caps that at one change per ~800 ms per channel, and a connection that cannot decide which band it is in is itself something the operator needs to hear. The limit remains per channel, so connecting eight igniters in succession still gives eight signals.
...
**Continuity-loss disarm IS implemented as of v1.35 (2026-08-26).** A transition to OPEN **or SUSPECT** (v1.71) on the currently armed channel while in ARMED or PRE_FIRE SHALL trigger the disarm sequence of §7.2.7, which see for the full requirements and for the exclusions that keep FIRING and POST_FIRE unaffected. This replaces the v1.8–v1.34 position that mid-arm igniter disconnection was an accepted low-probability risk.

Since the continuity circuit uses the SPDT relay NC contact, continuity readings are valid whenever the relay is de-energised, regardless of arm switch position. During FIRING, the fired channel reads OPEN (NC disconnected, saturated) while all other channels continue to provide live readings. After the fire pulse completes and the relay returns to NC, the fired channel's continuity reading automatically updates — OPEN (or SUSPECT) indicates the igniter has fired, CONNECTED/MARGINAL indicates a misfire.

**Post-fire igniter status:** After a fire pulse completes and the system returns to POST_FIRE/IDLE, the channel relay returns to NC position, reconnecting the igniter to the continuity sense circuit. The first valid continuity reading after POST_FIRE indicates whether the igniter has fired (OPEN, or rarely SUSPECT if residue leaves a finite resistance) or failed to fire (CONNECTED/MARGINAL). A delay of at least 50 ms after relay de-energisation SHALL be observed before the first ADC sample (relay dropout settling time).
...
- **The raw NACK reason code SHALL NOT be shown to the operator for a NACK answering a repeated CMD_FIRE** (fw 1.1.34). The NACK can beat the cause-carrying STATUS_UPDATE by tens of milliseconds (measured 7 ms), and the reason — typically `WRONG_STATE` — is true about the repeat and useless about the pad. The remote says "BASE ENDED SEQUENCE" and latches the armed channel; the first STATUS_UPDATE received in IDLE settles it one-shot with the §12.1 RM-07 discrimination (channel disarmed + band OPEN or SUSPECT → "CONTINUITY LOST - DISARMED", `BEEP_CONTINUITY_LOST`; anything else consumes the latch silently; SUSPECT added v1.71 — it disarms like OPEN and must be reported like OPEN). The latch is cleared by a new ARM attempt and by link loss, so it cannot name a continuity loss belonging to a previous sequence.
...
5. Selected channel has igniter continuity (band is CONNECTED or MARGINAL — OPEN and SUSPECT block arming, v1.71).
...
| ~~Orange (continuity SHORT)~~ → **Orange (continuity SUSPECT)** | (255, 140, 0) | Retired 2026-08-21 with the SHORT band; **revived v1.71** for the SUSPECT band's diamond glyph (◆) — same colour, unrelated meaning |
...
**As-built deviation (recorded 2026-08-27; SUSPECT added v1.71):** the implemented continuity palette is a green/yellow/orange family, not the blue-based table above — dark green CONNECTED (●), light green MARGINAL (▲), yellow OPEN (○), orange SUSPECT (◆) — matching `RLC_COLOR_CONT_*` in `rlc_config.h` and the README. Colour-blind accessibility is carried by the shape coding (●/▲/○/◆) rather than by the blue-for-CONNECTED substitution.
...
- **No device on this bus may use hardware CS while another shares the pin.**
  ESP-IDF gives each device its own hardware CS signal and routes it to the
  requested pin, so a second `spi_bus_add_device()` with the same
  `spics_io_num` steals the pin from the first — silently. Where more than one
  device is needed (it is, for the read clock below), **every** device SHALL
  set `spics_io_num = -1` and CS SHALL be driven by hand around each
  transaction. This is not a behaviour change from hardware CS, which already
  asserted and released per transaction; it only moves who does it.
...
**Register reads SHALL run at `DISPLAY_SPI_READ_CLOCK_HZ` (10 MHz), not at the
pixel clock (v1.69, firmware 1.2.18).** The ILI9488 write cycle is 50 ns
(20 MHz) but its *read* cycle is 150 ns (6.7 MHz): on a read the panel drives
MISO and its output delay is far slower than its input setup requirement. One
clock served both through 1.2.17 — already outside the read spec at 20 MHz,
and getting away with it.

At 40 MHz it stopped. The panel ID read back **`0x3F603B80` against
`0x2A403300`**, same board, minutes apart.
...
both ends were wrong in the same way and the check reported a healthy panel
while reading noise; the §5.5.6 test that rejects `0x00000000`/`0xFFFFFFFF` as
an undriven bus was judging a value the panel never sent. That check exists to
catch a dead panel on a unit that fires igniters, and a check that cannot
distinguish a live panel from a garbled read is not a check.

Reads therefore use a **second SPI device on the same bus at 10 MHz**, which is
only safe because of the manual-CS rule above — `spics_io_num = -1` on both
devices, CS bracketed around every transaction. 10 MHz rather than 6.7: a wide
margin the safe side of the read spec relative to 40 MHz, and a clean divisor
of the 80 MHz source. Reads are two per boot plus one per
`DISPLAY_HEALTH_INTERVAL_MS`, so nothing is gained by running them nearer the
edge. Verified on target: the ID reads `0x2A403300` with the band at 40 MHz.
...
not reach the built config — are all developer-facing. This is the only one an
operator standing at a firing point can see, and a fault-injection build lies to
its operator by construction.

**This SHALL NOT be read as covering a fault-injection base.** The remote knows
only its own build. The base does not advertise `CONFIG_RLC_FAULT_INJECTION` on
the wire, so no remote-side indication is possible for it; base firmware
provenance is verified at the bench, not at the pad.
...
- **Channel grid:** all 8 channels displayed with continuity band indicators. Filled circle (●) = CONNECTED. Triangle (▲) = MARGINAL (with label). Empty circle (○) = OPEN. Diamond (◆) = SUSPECT (with label) — **the glyph retired with SHORT is revived v1.71** for the new band; same diamond, unrelated meaning, four years and a band-merge apart (§5.4.2). The selected channel is highlighted with a `►[ CH N ]◄` cursor. Shape carries the meaning as well as colour, so the grid stays readable regardless of colour vision. **Four bands since 2026-09-14 (v1.71)**. As-built colours are dark green / light green / yellow / orange, not the blue-based palette above; see the §10.2.0 deviation note.
...
- Continuity OPEN: red.
- Continuity SUSPECT: orange (with "SUSPECT" label below the channel indicator) — v1.71.
...
| SUSPECT | `◆ SUSPECT - CHECK` | orange | Resistance too high to fire but finite — residue, or a lead/clip fault. Not "fired", not "connected": investigate (v1.71) |
...
#### 10.2.7a Persistent Condition Indication (v1.60)

A **transient overlay announces an event; a persistent condition SHALL also be
shown persistently.** The 3-second toast of §10.2.7 fires once, on the edge, and
is the right mechanism for a refusal or a NACK — something that happened. It is
the wrong mechanism on its own for a condition that continues to hold, because
the operator who was not looking at the display when it fired has no way to
recover the information.

This applies today to the arm-key contact fault (§5.5.2), and SHALL be the rule
for any comparable condition added later:

| Element | Behaviour while the fault stands |
|---|---|
| Toast | ARM KEY SWITCH FAULT on the rising edge, ARM KEY OK on the falling edge (§10.2.7 mechanism, amber) |
| Status band, REMOTE field | Reads **KEY FAULT** in place of ARMED/SAFE, on the main and ARMED screens |
| Main screen prompt line | Reads **ARM KEY FAULT - CHECK SWITCH**, ranked below a base error and above the next-step prompt |
| Refusal messages | The ARM and FIRE key-off refusals name the fault instead of instructing the operator to turn the key (§8.2.3, §8.2.4) |

The REMOTE field is the load-bearing part: its purpose is to report the key
position, and while the two contacts disagree the remote **cannot honestly claim
one**, so it must stop asserting rather than pick the more likely answer. It
also keeps the fault visible when a base error owns the prompt line.

The §10.2.6 latched error screen SHALL NOT be used for this class of fault. It
holds until reboot, which is right for a terminal error and wrong for a
maintenance warning on a path that is not the fire path — it would make the
indication more disruptive than the condition, and this fault is explicitly not
an interlock (§5.5.2).
...
| `BEEP_CONTINUITY_LOST` | 200 on, 100 off, 200 on, 100 off, 200 on | Continuity → OPEN/SUSPECT disarm (distinctive pattern; SUSPECT added v1.71) |
...
| `SIREN_CONTINUITY_LOST` | 200 ms on, 200 ms off, 3 cycles, then silence | Continuity-loss disarm (armed channel → OPEN or SUSPECT, v1.71) — distinctive alert |
| `SIREN_IGNITER_CONNECTED` | Single 100 ms blip, then silence | An igniter has been connected **and reads good** (added v1.57, fw 1.2.4) — a channel's continuity band moved to CONNECTED, so the operator at the pad hears the connection without checking the LEDs. Sounded in **BOOT and IDLE only**, never while the siren is already sounding (see §7.3.1 step 5 for the gate and its suppressions). |
| `SIREN_IGNITER_MARGINAL` | Two 100 ms blips (100 ms apart), then silence | An igniter has been connected **but reads MARGINAL** (added v1.58, fw 1.2.5) — high resistance, may not fire. One blip means good, two mean look at that crimp. Same gate and suppressions as `SIREN_IGNITER_CONNECTED`. Deliberately two short blips, not three long ones: it must not be heard as `SIREN_ERROR` or `SIREN_CONTINUITY_LOST`, which mean stop. OPEN and SUSPECT (v1.71) remain silent on both patterns — silence already means "not usefully connected", and the operator's next move is the same either way: look at the display. |
...
| `CONT_ADC_FULLSCALE_MV` | 950 | ADC full-scale at 0 dB attenuation. An open channel rests at ~3.19 V and saturates below this ceiling — saturated means OPEN, and `CONT_SUSPECT_UV` must stay below full scale by more than its hysteresis (v1.71) |
...
| `CONT_OPEN_UV` | 586000 | Threshold: above this the channel cannot fire (µV) — first the SUSPECT band, then OPEN (v1.71; until v1.70 this was directly the OPEN band). Corresponds to ~500 Ω through the 217 Ω sense branch. With hysteresis ±50000 µV. |
| `CONT_SUSPECT_UV` | 928000 | Threshold: above this = OPEN band (µV); below it down to `CONT_OPEN_UV` = SUSPECT (v1.71). Corresponds to ~1.09 kΩ — deliberately just under the raw-4000 point, leaving 12 mV of margin to the 950 mV full-scale ceiling so a saturated (truly open) reading still crosses threshold + hysteresis. With hysteresis ±10000 µV, narrow for the same reason (§5.4.2). |
| `CONT_HYSTERESIS_SHORT_UV` | 200 | Hysteresis band for SHORT/GOOD boundary (µV) — deprecated with `CONT_SHORT_UV` |
| `CONT_HYSTERESIS_MARGINAL_UV` | 5000 | Hysteresis band for CONNECTED/MARGINAL boundary (µV) |
| `CONT_HYSTERESIS_OPEN_UV` | 50000 | Hysteresis band for MARGINAL/SUSPECT boundary (µV) |
| `CONT_HYSTERESIS_SUSPECT_UV` | 10000 | Hysteresis band for SUSPECT/OPEN boundary (µV), added v1.71. Narrow by necessity: the OPEN side of this boundary is pinned at full scale (950000 µV), so threshold + hysteresis must stay below it |
...
| T-A04 | ARM channel with OPEN continuity | NACK with reason 0x04. **Re-verified 2026-09-15 (fw 1.2.20, v1.71):** OPEN still NACKs 0x04 and is distinct from the new 0x10 HIGH RESISTANCE for SUSPECT — the two refusals name different faults. |
...
| T-A09 | Verify continuity bands visible with arm switch OFF | All 8 channels show the correct continuity band (CONNECTED / MARGINAL / SUSPECT / OPEN) on the remote display regardless of base key switch position — continuity runs from its own 3.3 V supply through the relay NC contacts and does not depend on the arm relay. Use known resistors: **< ~67 Ω → CONNECTED, ~67–500 Ω → MARGINAL, ~500 Ω–1.09 kΩ → SUSPECT, > ~1.09 kΩ or nothing → OPEN**. (Band list corrected v1.37: SHORT was merged into CONNECTED on 2026-08-21, so a 0 Ω load reads CONNECTED, not SHORT. SUSPECT added v1.71.) |
...
| T-A15 | ARM channel with SHORT continuity | **SUPERSEDED — not runnable.** The SHORT band was merged into CONNECTED on 2026-08-21 (bug #26): the distinction sits below the measurement floor at the specified 1 mA test current, so reporting it would be guessing. `rlc_continuity_class.c` classifies into four bands (since v1.71) and `CONT_SHORT_UV` is unused. A 0 Ω load arms normally and displays as CONNECTED. No orange SHORT warning exists to observe. Retained for numbering continuity; the arming-with-a-very-low-resistance case is covered by T-A01. |
| T-A16 | **Disconnect the igniter on the armed channel while ARMED** | Base disarms within ~1 s of the band reaching OPEN: arm relay de-energised, all channel relays NC, siren off, state → IDLE, `STATUS_UPDATE` shows that channel OPEN and nothing armed. Base log shows `Continuity OPEN on armed ch N during ARMED — disarm`. (v1.35) **Re-verified 2026-09-15 (fw 1.2.20)** after the v1.71 edit to `armed_channel_went_open()` (which now also accepts SUSPECT): pull-while-ARMED disarms within ~1 s with the 3 × 200 ms blasts; the key-off disarm was re-run alongside and remains acoustically distinguishable. |
...
| T-A23 | **Arm key switch contact cross-check** (v1.59, §5.5.2) | Normal operation first: turn the key slowly ARM↔SAFE several times — the arm LED and the remote's key state follow as before and **no fault is raised**, because break-before-make disagreement lasts far less than `ARM_SWITCH_DISAGREE_MS`. Then the fault: with the remote powered, disconnect the **NC** lead (GPIO 2) and leave the key at SAFE — after ~1 s, triple beep, display **ARM KEY SWITCH FAULT**, log line naming both contacts. Reconnect: **ARM KEY OK**. Then prove it is not an interlock — with the NC lead still disconnected, confirm the key **still arms normally** on the NO contact and a full arm/fire sequence is unaffected. Finally the case the feature exists for: disconnect the **NO** lead (GPIO 7) and turn the key to ARM — the remote correctly refuses to arm, but now also reports ARM KEY SWITCH FAULT instead of silently insisting the key is at SAFE. **v1.60 additions:** while any of the above faults stands, confirm the status band's REMOTE field reads **KEY FAULT** (not ARMED/SAFE) on the main screen and, if reachable, the ARMED screen; that the main prompt line reads **ARM KEY FAULT - CHECK SWITCH**; and that a long-press with the NO lead disconnected is refused with **ARM KEY FAULT - CHECK SWITCH**, not TURN ARM KEY FIRST. Wait out the 3 s toast first — the point of these is that they are still there afterwards. |
| T-A24 | **SUSPECT band** (v1.71, §5.4.2) | With the base in IDLE, connect a mid-window resistor (~750–820 Ω, or a deliberately corroded/high-resistance joint) to a channel: the remote grid shows **◆ SUSPECT in orange**, the strip pixel is steady orange, and the base log reports the band as SUSPECT — **no blip sounds** (silence is the specified treatment, §12.2). Long-press ARM on that channel: NACK `0x10`, remote displays **"HIGH RESISTANCE ON CH N"** — not "NO CONTINUITY". Then the disarm half: arm a *good* channel, and swap its load for the high-value one — the base disarms within ~1 s (§7.2.7), siren `SIREN_CONTINUITY_LOST`, remote shows the channel ◆ SUSPECT. **The swap must have no open gap** — bridge the high-value resistor across first, then remove the good load; simply unplugging the igniter exercises the OPEN path (T-A16) and proves nothing about SUSPECT. Boundary sanity: a ~390 Ω load reads MARGINAL (below the 500 Ω boundary) and a ~1.2 kΩ load reads OPEN (above saturation) — confirming the SUSPECT window is where specified and neither wider nor narrower. **PASS 2026-09-15 (fw 1.2.20, both units), as-run values:** 820 Ω (~783 mV, mid-window) → ◆ orange, silent, log `band 0 -> 3`; ARM → NACK `0x10` "HIGH RESISTANCE"; arm-then-swap (820 Ω bridged across a 2 Ω load, 2 Ω then removed) → disarm within ~1 s, 3 × 200 ms blasts, "CONTINUITY LOST - DISARMED", channel settles ◆ SUSPECT — the CONNECTED→SUSPECT disarm path live on target; 390 Ω (~510 mV) → ▲ MARGINAL with the two-blip signal. Boundary points recorded the same day: 1 kΩ → ◆ SUSPECT, 1.2 kΩ → ○ OPEN (the split sits exactly between them, matching the ~1.09 kΩ derivation), and a 560 Ω probe noted as reading ◆ SUSPECT (~625 mV, just inside the window) — do **not** substitute 560 Ω for the MARGINAL check. |
...
| T-L02 | LED strip renderer | Continuity map colours: CONNECTED/MARGINAL/SUSPECT/OPEN resolve to `RLC_COLOR_CONT_*` on the correct pixels (SUSPECT → steady orange, v1.71). |
...
| T-U10 | Continuity band classification | Feed known microvolt values: 0, 300, 500, 1000, 30000, 66000, 100000, 500000, 600000, 900000, 925000, 935000, 950000, 1500000, 2000000, 3190000. Verify correct band assignment (**CONNECTED, MARGINAL, SUSPECT, OPEN** — four bands since v1.71) at each threshold, including both sides of the SUSPECT/OPEN boundary and the saturated full-scale reading classifying OPEN. |
| T-U11 | Continuity hysteresis | Feed voltage sequence oscillating near each threshold boundary. Verify no spurious band transitions within the hysteresis band — including the narrow ±10000 µV SUSPECT/OPEN band, where a saturated 950000 µV reading must still cross into OPEN (v1.71). |
| T-U12 | Continuity bands encoding | Verify 2-bit-per-channel packing into uint16: ch1 in bits 1:0 through ch8 in bits 15:14. Verify extraction for all band combinations. Verify that enum values (`CONT_OPEN`=0, `CONT_CONNECTED`=1, `CONT_MARGINAL`=2, `CONT_SUSPECT`=3) match wire encoding directly with no mapping required. Value 3 is SUSPECT since v1.71 (the retired SHORT slot, repurposed; no fold-on-decode remains — strict version matching guarantees a linked peer understands it). |
...
| T-FSM10 | §12.2 igniter-connection blips: CONNECTED sounds one and MARGINAL two, in IDLE and BOOT; OPEN silent; silent for an `initial` classification and inside the post-fire inhibit window; the per-channel **per-band** rate limit suppresses a repeat but lets a MARGINAL→CONNECTED re-seat sound inside the window; eight channels still give eight signals; **both patterns gated out of ARMED (with the continuous tone asserted still sounding), PRE_FIRE, FIRING, LINK_LOST, ERROR and POST_FIRE** |
...
| LINK_REQUEST while already linked (same MAC) | Remote rebooted | If base is in IDLE or LINK_LOST: base generates new session token, responds normally (session reset). Not an error. **If base is in ARMED, PRE_FIRE, FIRING, or POST_FIRE: LINK_REQUEST is rejected with `LINK_REJECT_BUSY` (0x02)** — no session reset, and the armed/firing state is unaffected. Session reset during safety-critical or cooldown states is blocked to prevent disruption. The remote's retry mechanism will deliver a subsequent request after the base returns to IDLE. |
