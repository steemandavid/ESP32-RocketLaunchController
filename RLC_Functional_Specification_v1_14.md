...
5. **Igniter-connected blip (v1.57, §12.2 `SIREN_IGNITER_CONNECTED`).** On a transition to **CONNECTED**, the FSM SHALL sound a single `SIREN_CONNECT_CHIRP_MS` blip so that the operator working at the pad hears the connection being made. This is audible-only: it changes no state and gates nothing.

   The blip SHALL sound **only in BOOT and IDLE**. BOOT is included deliberately — igniters are routinely connected before the LCO has powered the remote, and the base waits in BOOT until the link comes up. It SHALL NOT sound in ARMED, PRE_FIRE or FIRING, where the siren is the pad's continuous warning that the fire path is live and where the one-shot's drive-on/drive-off mechanism would silence it; nor in LINK_LOST or ERROR, where a patterned alert is running and carries meaning; nor in POST_FIRE, where the fired channel's re-read is the fire-result indication rather than an operator action. The siren driver SHALL additionally decline the blip whenever the siren is already sounding — the state gate and the driver guard are both required, so that the pad warning cannot be silenced by a property of one switch statement.

   Only CONNECTED sounds. MARGINAL remains informational (step 2) and silent: it is a connection the operator should inspect rather than trust, and giving it the same blip as a good connection would teach the wrong reflex.

   Three suppressions keep the blip meaning exactly "somebody just connected an igniter": an event carrying the `initial` flag never sounds; a channel SHALL NOT blip twice within `SIREN_CONNECT_CHIRP_MIN_INTERVAL_MS` (chatter from a half-seated connector — the limit is per channel, so connecting eight igniters in succession still gives eight blips); and no blip SHALL sound within `SIREN_CONNECT_CHIRP_INHIBIT_MS` of the POST_FIRE → IDLE transition, where an igniter that failed to fire reappears as CONNECTED with nobody having touched it (see the post-fire igniter status paragraph below).
...
| `SIREN_CONNECT_CHIRP_MIN_INTERVAL_MS` | 2000 | Minimum interval between two connect blips **on the same channel** — anti-chatter for a half-seated connector. Per channel, so eight igniters connected in succession still give eight blips. |
...
| T-FSM10 | §12.2 igniter-connected blip: sounds in IDLE and BOOT; silent for MARGINAL and OPEN, for an `initial` classification, inside the per-channel rate limit, and inside the post-fire inhibit window; per-channel limit still gives eight blips for eight channels; **gated out of ARMED (with the continuous tone asserted still sounding), PRE_FIRE, FIRING, LINK_LOST, ERROR and POST_FIRE** |
