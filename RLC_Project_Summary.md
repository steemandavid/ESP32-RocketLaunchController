...
- **The base tells you when an igniter is connected**, by sounding a short siren blip on each connection (September 2026, at the operator's request). It means the person kneeling at the motors no longer has to walk back to the base or shout to the firing point to confirm a connection was made. The blip sounds only while the base is idle: once a channel is armed the siren is sounding continuously to warn that the pad is live, and nothing is allowed to interrupt that. A *marginal* connection — one with too much resistance to trust — sounds **two** blips instead of one, so the operator can tell a good joint from a doubtful one by ear and re-make it on the spot; the single blip that follows is the confirmation that the fix worked. Both are short and quiet enough that they cannot be mistaken for the siren's alarm patterns, which are longer and mean stop.
