...
| 5 | Hardening and Final Testing | **COMPLETE (release fw 1.2.0, 2026-08-28)** — safety tests §15.4 mostly PASS, review fixes applied and verified **on target** (MAJ-01, CRIT-01 both halves, bug #29 regression suite T-A16/T-A17/T-A18 all PASS); final docs sweep done, final-build audit clean (zero injection/harness symbols in both stock ELFs), both units on stock 1.2.0 and linked (fw **1.2.18** since 2026-09-14 — 1.2.1 added the remote fault-injection splash banner; 1.2.2 removed the main-screen continuity legend and enlarged the status band; 1.2.3 added the base's one-chirp boot-complete siren test; both flashed together, link verified. **fw 1.2.4, 2026-09-10** adds the igniter-connected blip `SIREN_IGNITER_CONNECTED` and **fw 1.2.5, same day** its marginal counterpart `SIREN_IGNITER_MARGINAL` (two blips) — base-only, audible-only; both units flashed and linked, **T-A21 and T-A22 PASS** on target. **fw 1.2.6, same day** records the remote arm key's previously undocumented NC contact on GPIO 2, claims the pin and cross-checks the two contacts, and **fw 1.2.7, same day** makes that fault persist on the display and renames the refusals that used to tell the operator to turn an already-turned key — both units flashed and linked; **T-A23 outstanding**). **Deferred past release** (tracked here, not blocking live fire): T-S10b, T-S12/S13, T-S18 (physical access), T-C06 replay tool, range 10–100 m, power consumption, remote FSM host harness, CI runner |
...
|---|---|---|---||
| 1.2.19 | 2026-09-14 | both | **Fixes for the RLC-REVIEW-ALL-010 full-project review** (`Code_Review_AllPhases_20260914_1451.md`; FSD v1.70). Verdict was PASS WITH NOTES — fire path verified sound, zero regressions — with two Majors, both in the remote's `wait_for_ack()`: **R-MAJ1**, `EVT_DISPLAY_FAULT` was silently consumed during the ARM/FIRE ACK wait (the display posts it exactly once per power cycle, so the §5.5.6 disarm+ERROR never ran anywhere — a fire sequence could proceed on a dead panel), now handled inline like `EVT_LINK_LOST`; **R-MAJ2**, the inline battery-critical entered terminal ERROR without CMD_DISARM/CEASE_FIRE — the one path the 1.1.35 CRIT-01 'b' fix never covered; with the ARM ACK lost on an accepted arm the base held the relay for its full 10 s ARM_TIMEOUT — now mirrors the ARMED handler. Also: `ERR_WATCHDOG_RESET` is set at last (`esp_reset_reason()` at boot; a TWDT reboot shows as WATCHDOG RESET, not err=0x00); link-loss during arm-verify NACKs `NACK_COMM_DEGRADED` (last unanswered §7.2.2 canceller); dedicated MARGINAL-on-armed-channel advisory; CMD_FIRE with arm sense LOW NACKs `ARM_SENSE_FAULT` (the true fault); display-fault send is deferred-and-retried, not lost on a full FSM queue; first-frame splash decode failure leaves the plain band; buzzer up before the self-tests; `do_disarm_and_idle()` re-syncs the encoder (INF-06 closed); synchronous `esp_now_send()` errors count toward §6.4.1a; send-failure notification is an atomic counter; duplicate LINK_ACK no longer re-resets the session (CM-08 closed); stale comments corrected (rlc_base_fsm.h order, "allow 0 seq", guard-callback docs); recv trampoline and dead `DISPLAY_ROTATION` removed; `rlcv_repack.py` validates input; `build_remote.sh splash` checks partition table and asset size. FSD v1.70 also brings six spec-text passages level with the code (§14.4 clocks, dead-man stamping, edge beep, §8.2.3 refusal feedback, §9.10 task table, §9.13 remote boot order). Host tests 497 checks 0 failures; both units build clean. **Both units flashed and linked** (banner-verified v1.2.19 on each; LINK_REQUEST/ACK handshake, base BOOT → IDLE, remote LINKING → IDLE). |
| 1.2.18 | 2026-09-14 | remote | **Panel-ID read-back fixed at 40 MHz; CS driven manually on both SPI devices** (FSD §10.2.1, v1.69). Closes the defect 1.2.17 shipped with. The ILI9488 read cycle is 150 ns (6.7 MHz) against a 50 ns write cycle, so register reads do not survive the 40 MHz pixel clock — the ID came back `0x3F603B80` instead of `0x2A403300`. **The corruption was stable**, which is what made it a defect rather than noise: `display_health_check()` compares each read against the boot read, so both ends were wrong the same way and it reported a healthy panel while reading noise, while the §5.5.6 undriven-bus test judged a value the panel never sent. That check guards a unit that fires igniters. Reads now use a second SPI device at new `DISPLAY_SPI_READ_CLOCK_HZ` (10 MHz), which is only safe because **CS is now driven by hand on both devices** (`spics_io_num = -1`, bracketed in `spi_xfer()`/`spi_xfer_rd()`) — not a behaviour change, since hardware CS already asserted per transaction, it only moves who does it. v1.68's rule "exactly one SPI device handle" is replaced by the precise one: no device may use hardware CS while another shares the pin. Verified on target — ID reads `0x2A403300` with the band at 40 MHz, health checks passing, both units linked (rssi −34). ||
| 1.2.17 | 2026-09-14 | remote | **Splash band becomes the top half of the panel with the title and credit overlaid on it** (FSD §10.2.1, v1.68, operator request). Release of the 1.2.12–1.2.16 work. The band goes 480x80 at `y 112` → 480x128 → **480x160 edge to edge at `y 0..159`**, the title becomes one line **`ROCKET LAUNCH CONTROLLER`** (dropping "ESP32 WIRELESS" from the boot screen, as asked, and agreeing with the firmware-mismatch screen) and the club credit moves to the foot of the band, leaving **rows 56..119 text-free and full strength** for the touchdown; the asset is re-framed (`--track-bias-y -0.08`) to land it there. Text over footage is carried by two **baked-in** ramped scrims plus an eight-neighbour glyph outline — the grading cap stays at `0x9A`, since darkening the whole clip to suit its worst frame costs the picture everywhere to fix a problem confined to the text rows. **The budget turned out to be CPU, not just wire:** `flush()` transmits by polling, so transfer time is core time — measured at 480x128/20 MHz it cost 99 ms of which only 74 ms was the transfer, the rest per-row `memcmp`, bounce copy and shadow update (decode 12 ms, blit 9 ms, neither ever mattered). Hence **5 Hz**, below the display rate, with early-outs so frames where the clip has not advanced cost almost nothing: 73 ms average against ~120 ms when it advances. `DISPLAY_SPI_CLOCK_HZ` goes to **40 MHz**, above the ILI9488's specified 20, which is what makes 480x160 fit at all. Scrims baked into the asset make the firmware's `VBAND_SCRIM_*` a contract with `mkvideoband.py` — **and the one asset mismatch that does not fail safe**, the validator checking dimensions but not scrim geometry. Tooling: `mkvideoband.py` now seeks to `--start` instead of decoding from t=0 (~6 min → ~30 s over the 4K source, which is what made framing iteration possible) and bakes both scrims; new `tools/rlcv_repack.py` re-times a band losslessly, because sampling at 5 Hz re-tracks rather than re-times. ||
| 1.2.12–1.2.16 | 2026-09-14 | remote | Development steps toward 1.2.17, each flashed and looked at on the panel; see `rlc_version.h` for the full narrative. Two are worth carrying forward. **A latent task-starvation bug, found as a reboot loop (1.2.12):** `xTaskDelayUntil()` returns `pdFALSE` *without blocking* when its deadline has passed, and re-basing to now leaves the next frame to overrun identically — so a *consistently* slow frame degenerates into a loop that never yields once, `display_task` (prio 2) starved `buzzer_task` (prio 1) on core 1, and the remote rebooted on the 5 s task watchdog ~6 s into every boot. Fixed by an explicit `DISPLAY_FRAME_MIN_YIELD_MS` yield after any overrun, bounding the display's share of the core regardless of frame cost — a safety property, since the buzzer is how the remote talks to an operator when the screen is the thing that is wrong. **A silent CS-routing trap (1.2.12, withdrawn):** a second `spi_bus_add_device()` sharing `spics_io_num` **stole the CS pin from the first device**; every write ran with CS unasserted, the panel ignored its init sequence and sat white, while reads on the second handle reported a healthy, correctly-identified panel — the logs looked clean because the only path still working was the one being logged. Hence the standing rule that a clock or bus change is **judged by looking at the panel, never by the boot log**. ||
| 1.2.11 | 2026-09-12 | remote | **The splash video band loops; tracking and framing reworked** (FSD §10.2.1, v1.66, operator request). The clip had held its last frame — a deliberate choice, since `STATE_LINKING` maps to the splash screen and an unlinked remote sits on it indefinitely. The tradeoff is now recorded rather than left to be re-litigated: a held frame was **free** (decode skipped, blit writing identical pixels that §10.3's per-row `memcmp` rejected), while looping costs a JPEG decode plus ~46 ms of band over SPI every 100 ms for as long as the remote is powered without a base — within budget, being the same load the first ten seconds already carried, but permanent, and sitting alongside `BEEP_LINK_TRY` whose whole purpose is getting an idle unlinked remote switched off. Also two framing defects reported against 1.2.10, each with a distinct cause: **too much ground** because tracking selected the area-weighted centroid of the bright blobs, which is right for two symmetrical plumes and wrong for a smoke column (the centroid is dragged into the dense base of the cloud) — now the bounding-box centre; and **cloud tops cut** because the zoom held its peak, while the subject keeps growing after touchdown — now a three-point move, wide, in to the touchdown, back out. The pull-back earns its keep twice, since ending near the opening width is also what stops the loop wrapping with a visible jump; FSD §10.2.1 now requires that of any looping asset. Tracking also gained a search window (only blobs near the current position are considered), which keeps it on thinning smoke instead of jumping to sunlit roads near the horizon. **Both units flashed 1.2.11 and linked** — `LINK_ACK accepted`, LINKING → IDLE, rssi −61, 0 missed pings, 0 tx failures. Remote-only, display, no protocol change. ||
| 1.2.10 | 2026-09-12 | remote | **Splash layout and cut corrections** (FSD §10.2.1, v1.65, operator request). Three fixes to 1.2.9. **(1) The cut missed the touchdown** — its crop pan settled *after* the landing, putting the pads on the band's bottom edge so the boosters descended out of the strip rather than landing in it. The source camera tracks and zooms, so the ground line runs ~63-65% of frame height at t=226-227 s and rises to ~55% by t=228 s; the band has to suit the earlier, lower line and still hold the later one. Recorded in `assets/README.md` so the next re-cut does not rediscover it. **(2) The band had no margins** — butted against the club credit above and the status line below, it read as a rendering fault rather than a frame. Now 15 blank rows each side, stated as load-bearing. **(3) The version string left its own row** for the copyright line (`(C) 2026 David Steeman  v1.2.10`), which is what paid for those margins; it stays on the boot screen because the strict version check makes the running firmware something an operator must be able to read without a serial cable. The fault-injection banner is keyed off the band geometry instead of its own literals. Remote-only, display, no protocol change. ||
| 1.2.9 | 2026-09-12 | remote | **The boot splash plays real footage** (FSD §10.2.1, v1.64, operator request). 1.2.8's procedurally-drawn boosters were rejected on sight and their code is removed entirely. In their place a letterboxed **480x80 JPEG frame sequence at 10 Hz** — the Falcon Heavy side boosters landing at LZ-1/LZ-2, from NASA imagery that is public domain in the US. **Full-panel video is impossible, not merely slow**: the ILI9488 is 18-bit-only over SPI, so a 480x320 frame is 460,800 B = 184 ms at `DISPLAY_SPI_CLOCK_HZ` against a 100 ms frame period, and it would saturate the panel for exactly the window the link handshake runs in. The band is 115,200 B = 46 ms. Decode cost and flash space were never the constraint; the wire is. The asset lives in a **new `splash` partition, not the binary**, so footage can be re-cut and reflashed without touching the image that runs the fire path — which required the remote off `CONFIG_PARTITION_TABLE_SINGLE_APP` (its 1 MB factory partition held an 863 KB app) onto `partitions_remote.csv`, 3 MB factory + 2 MB splash, memory-mapped, no filesystem. **The base keeps the single-app table**, being out of scope. `build_remote.sh flash` now writes bootloader + partition table + app, and gains `splash <file>`. New `tools/mkvideoband.py` builds the container and performs the mandatory grading (desaturate, darken, hard-cap every channel at `0x9A`) so the white title text keeps its contrast. **Every failure degrades to a plain dark band and a normal boot** — missing partition, missing/blank/corrupt asset, wrong dimensions, undecodable frame; the container is validated whole at init so the 10 Hz path can index the frame table without re-checking bounds, and an erased all-`0xFF` partition is rejected as cleanly as a corrupt one. Verified on target both ways. Remote-only; display, partition layout and flash procedure. ||
| 1.2.8 | 2026-09-12 | both | **Splash retry counter, handshake backoff, and an unlinked-remote blip.** The splash's animated background (a procedurally-drawn booster landing) was rejected and replaced in 1.2.9; the rest stands. **(1) The splash stopped claiming the handshake was bounded.** It read `Attempt N / 5`, clamped, bar pinned at 100%, while the firmware retried forever — `LINK_REQUEST_MAX_RETRIES` is a *backoff threshold*, not a give-up count. A boot screen frozen while the unit is still working reads as a hung remote, and because `STATE_LINKING` maps to the splash this was the **steady-state display of a remote that could not find its base**, not a boot-time glitch. Now an unbounded `Attempt N`, a `No response from base` headline and an indeterminate sweep past the threshold; `display_splash()` loses its `max_attempts` argument. **(2) `LINK_REQUEST_SLOW_INTERVAL_MS` became a real interval.** It had read 2000 in `rlc_config.h` since the initial scaffolding commit — equal to the fast interval — so §6.4.1's backoff ternary chose between two identical values and **had never once taken effect**, and the FSD table had been edited down to 2000 to match the code rather than the code fixed to match v1.3–v1.8. Restored to 5000; the fast phase is untouched, so v1.14's aggressive-retry intent stands. Confirmed on target: attempts 1-5 at 2 s, then 5000 ms apart. Costs up to 3 s of extra reconnect latency in LINK_LOST, accepted because the base fails safe independently and the remote is already alarming. **Explicitly not a battery saving** — the radio is `WIFI_PS_NONE` with no PM or tickless idle, so the receive chain draws continuously whatever the transmit cadence; recorded as such in three places so it is not cited as one later. **(3) New `BEEP_LINK_TRY`** (§12.1) is the actual battery measure: a 40 ms blip per handshake attempt while `LINKING`, so a remote left switched on with no base in range says so audibly instead of flattening its pack behind a screen nobody is looking at. `LINKING` only — `LINK_LOST` already sounds a continuous alarm, and a one-shot layered under a running alarm only contends for the pattern player. ||
| 1.2.7 | 2026-09-10 | remote | **The arm-key fault is now visible where it is needed** (FSD §10.2.7a, v1.60). 1.2.6 announced ARM KEY SWITCH FAULT with a triple beep and a 3 s toast, once, on the edge — a transient announcement for a condition that persists until the switch is serviced. Found by asking whether an explicit error screen existed: it did not, and the toast was landing at the wrong moment. The exposing sequence: the operator turns the key, looks at the pad not the screen, long-presses, and is refused with **"TURN ARM KEY FIRST"** — told to turn a key they are looking at, already turned, at the exact moment they are asking why it will not arm, the explaining toast long expired. **(1) Refusals are state-aware** — the ARM guard and the FIRE key-off guard say "ARM KEY FAULT - CHECK SWITCH" / "ARM KEY FAULT - FIRE REFUSED" while the contacts disagree; the refusals themselves are unchanged and still correct, since a NO contact failing open SHOULD read as SAFE and fail safe. **(2) The indication persists** — the status band's REMOTE field reads **KEY FAULT** instead of ARMED/SAFE on the main and ARMED screens, and the main prompt line reads ARM KEY FAULT - CHECK SWITCH (ranked below a base error, above the next-step prompt, which is what the fault makes untrustworthy). The REMOTE field is the load-bearing half: its job is to report the key position and, with the contacts disagreeing, the remote cannot honestly claim one. **Deliberately not the §10.2.6 latched error screen** — that holds until reboot, disproportionate for a maintenance warning off the fire path, and this is explicitly not an interlock. New §10.2.7a states the general rule for future conditions: a toast announces an event, a persistent condition must also be shown persistently. **Both units flashed 1.2.7 and linked.** T-A23 extended (**outstanding**). Remote-only, display and wording, no protocol change. ||
| 1.2.6 | 2026-09-10 | remote | **The remote's arm key switch is an SPDT and its NC contact is on GPIO 2** (FSD §5.5.2, v1.59). Reported by the operator against the pinout this project's own documents had given them: common at ground, NO → GPIO 7, NC → GPIO 2, and only the NO leg had ever been recorded. **FSD C.2 listed GPIO 2 among the remote's spare pins, "available for future expansion"** — while the NC contact hard-shorts it to ground for as long as the key sits at SAFE. Nothing had ever configured it, so the short was harmless, but the document was pointing at the one spare pin with a wire on it. Fixed three ways: the pin is **claimed** (`arm_switch_init()` configures it as a pulled-up input, so it cannot be reassigned by accident); the docs are corrected (§5.5.2 describes the SPDT, C.2 lists the NC input, **6 spare GPIOs not 7**, and the hw-test harness declares the pin too); and the contacts are **cross-checked** — complementary on a healthy switch, so a disagreement persisting past `ARM_SWITCH_DISAGREE_MS` (1000 ms, clear of the break-before-make gap) raises ARM KEY SWITCH FAULT on log, buzzer and display. That buys the diagnosis that was missing: with the NO leg alone, a broken wire or failed contact reads exactly like "key at SAFE", so the remote refused every long-press with TURN ARM KEY FIRST while the operator looked at a key they had already turned. **Deliberately not an interlock** — arming still follows the NO contact even while the fault is raised, because the NC joint has never been exercised and letting it veto arming would trade a silent diagnostic gap for a marginal joint disabling the remote at a launch; the remote's key is not in the fire path (§5.4.4) in any case. New test **T-A23 (outstanding)**. **Both units flashed 1.2.6 and linked** — remote logs `arm_sw: initialised (NO=7, NC=2, LED=8)`, base `LINK_REQUEST from remote fw 1.2.6`, rssi −21, no fault raised with the key at ARM (which is expected but not yet discriminating: at ARM the NC contact is open and reads the same as an unconnected pin — T-A23's SAFE-position and lead-disconnect steps are what actually prove the wiring). Remote-only, no protocol change. ||
| 1.2.5 | 2026-09-10 | base | **`SIREN_IGNITER_MARGINAL` — two blips for a marginal connection** (FSD §12.2, v1.58). The other half of 1.2.4's feature, deferred there until the single blip had been heard at the pad (T-A21 PASS, same day). MARGINAL was the one band that made no sound, so a high-resistance crimp — the fault the operator most wants told about while still standing at the motor — was indistinguishable from no connection by ear. Now **one blip = good, two = marginal, silence = nothing**. Two short blips rather than three long ones so it cannot be heard as `SIREN_ERROR`/`SIREN_CONTINUITY_LOST`, which mean *stop*; the FSD states that distinction as a requirement on any future change to either pair. **The rate limit became per channel AND per band**: a repeat of the same signal is still suppressed as chatter, but a *change* always sounds, because "two blips → re-seat → one blip" is the loop the feature exists to close and a band-blind window would have swallowed the confirming blip and left the operator believing the crimp was still bad. Accepted cost: a connection oscillating on the CONNECTED/MARGINAL boundary can blip per change, capped at ~800 ms per channel by the sampler — and an undecided joint is itself worth hearing. All v1.57 gates and suppressions apply unchanged to both patterns. Host tests T-FSM10 extended (150 checks); **on-target T-A22 PASS 2026-09-10** — two blips countable and not confused with the alarm, the re-seat confirmation sounds inside the window, same-band repeat silent, and no blips while ARMED with the continuous tone unbroken. **Both units flashed 1.2.5 and linked** (12/12 self-tests, `LINK_REQUEST from remote fw 1.2.5`, rssi −23/−24). Base-only, audible-only, no protocol change. ||
| 1.2.4 | 2026-09-10 | base | **`SIREN_IGNITER_CONNECTED` — a short blip each time an igniter is connected** (FSD §12.2, v1.57, operator request). A single 100 ms siren blip when a channel's continuity band reaches CONNECTED, so the person working at the motors hears the connection without walking back to read the LEDs on either unit. Sounds in **BOOT and IDLE only** — never in ARMED/PRE_FIRE/FIRING, where the siren is the pad's continuous fire-path-live warning and the one-shot's own drive-off would silence it, and never in LINK_LOST/ERROR, where a patterned alert is running. Enforced twice: the FSM state gate (§7.3.1 step 5) and `siren_chirp_connect()` itself declining whenever the siren is already sounding. CONNECTED only — MARGINAL stays silent (a connection to inspect, not to trust); a distinct MARGINAL pattern is deferred until this one has been heard in the field. Three suppressions keep the blip meaning one thing: a new `initial` flag on `EVT_CONTINUITY_CHANGED` stops igniters already connected at power-on blipping through the sampler's first sweep, a 2 s per-channel rate limit covers a half-seated connector, and a 2 s inhibit after POST_FIRE → IDLE covers an unfired igniter reappearing as CONNECTED. Host tests T-FSM10; **on-target T-A21 PASS 2026-09-10** — the blip itself and all six suppression cases, including the safety-relevant one (a connection on another channel while ARMED neither blips nor interrupts the continuous tone). **100 ms confirmed by ear at the pad** — a tone, not a click, and not punishing at arm's length — so `SIREN_CONNECT_CHIRP_MS` stands as specified. **Both units flashed 1.2.4 and linked** (base `44:1b:f6:81:f1:70` over `usb-1a86_USB_Single_Serial_5B5E042156-if00`, remote `ac:a7:04:e2:f2:8c` over `…5B5E043219-if00`; 12/12 self-test suites, `LINK_REQUEST from remote fw 1.2.4`, rssi −33/−34, vbat 12.16 V). Base-only, audible-only, no protocol change. ||
