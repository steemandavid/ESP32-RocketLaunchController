...
| B-C07 | Noise floor analysis | `cont 1 raw 256` with 2 Ω resistor, then ~20 consecutive `cont 1` reads. | **Criterion is on the averaged reading the classifier consumes** (`cont <ch>` averages `CONT_ADC_SAMPLES` = 64 samples, as production's `CONT_OVERSAMPLE_COUNT` does): stddev of consecutive averaged reads < 2 mV, mean matches expected ±5 mV. The `raw 256` per-sample stddev (~10 mV at 0 dB, measured 2026-09-15) is a diagnostic only — 64× averaging brings it to ~1.1–1.3 mV effective. |
...
idf.py build
idf.py -p /dev/serial/by-id/usb-1a86_USB_Single_Serial_5B5E042156-if00 flash monitor   # base COM port (chip #4 board, 2026-08-20)
```
