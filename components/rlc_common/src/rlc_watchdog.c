...
int rlc_watchdog_init(void)
{
    ...
    /* N3: this rebuilds the subscriber list, so it MUST run before any task
     * self-registers. See the header for what happens when it does not. */
    ...
    ESP_LOGI(TAG, "Watchdog reconfigured to %d second timeout (no tasks started yet)",
             WATCHDOG_TIMEOUT_S);
    return 0;
}
